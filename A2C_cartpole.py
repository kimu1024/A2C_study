...
class Net(nn.Module):
    ...
    def get_value(self, x):
        '''状態xから状態価値を求めます'''
        # self(x)でself.forward(x)が実行される。self(x)=Net(x)であり、Netはnn.Moduleクラスを継承している。
        # nn.Moduleクラスでは__call__()メソッドを持っており、その中でforward()を実行するように指定されている
        # __call__()は、オブジェクトが具体的な関数を指定されずに呼び出されたときに動作する関数
        value, actor_output = self(x)
